...
import {
  Contact, CONTACT_TYPES, TYPE_CONFIG, SCORE_CONFIG,
  getInitials, getRelationshipLabel,
  ContactModal, DeleteConfirmModal, InsightCards, DetailPanel,
} from "./components";
...
export default function ContactsPage() {
  ...
  function openEditModal(c: Contact, e: React.MouseEvent) { e.stopPropagation(); setEditingContact(c); setModalOpen(true); }

  async function handleSaveContact(formData: { firstName: string; lastName: string; email: string; phone: string; company: string; type: string; notes: string }) {
    setSaving(true);
    try {
      if (editingContact) await api.patch(`/contacts/${editingContact.id}`, formData);
  ...
  async function handleDeleteContact(id: string) {
    try { await api.delete(`/contacts/${id}`); setDeleteConfirmId(null); loadContacts(); }
  ...
  async function onImportFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      alert(`Imported ${result.imported || 0} contacts`);
      loadContacts();
    } catch { alert("Failed to import contacts"); }
  ...
  function renderContactCard(contact: Contact) {
    ...
    const overflow = (contact.tags?.length || 0) - 4;
        ...
        <article className="bg-surface-card rounded-lg border border-border-subtle p-5 hover:border-primary/30 transition-all flex flex-col h-full shadow-card hover:shadow-card-hover relative group">
          ...
          <div className="flex items-center justify-between mt-auto pt-3 border-t border-border-subtle">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={(e) => openEditModal(contact, e)} className="p-1 rounded hover:bg-gray-50 text-text-muted hover:text-primary transition-colors opacity-0 group-hover:opacity-100"><span className="material-symbols-outlined text-[16px]">edit</span></button>
              <button onClick={(e) => { e.stopPropagation(); setDeleteConfirmId(contact.id); }} className="p-1 rounded hover:bg-red-50 text-text-muted hover:text-red-600 transition-colors opacity-0 group-hover:opacity-100"><span className="material-symbols-outlined text-[16px]">delete</span></button>
            </div>
          </div>
        </article>
      </div>
    ...
    <div className="p-4 md:p-6 mx-auto max-w-[1600px] w-full flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-text-main tracking-tight">Contacts</h1>
          ...
          <p className="text-text-secondary text-sm flex items-center gap-2">
            ...
            {loading ? "Loading..." : `${totalContacts} contact${totalContacts !== 1 ? "s" : ""} in your network`}
          </p>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="relative">
            <button onClick={(e) => { e.stopPropagation(); setTypeOpen(!typeOpen); setSortOpen(false); setMoreOpen(false); }}
              className="flex h-9 shrink-0 items-center gap-2 rounded-lg bg-surface-card border border-border-subtle px-3.5 hover:border-primary/30 hover:shadow-sm transition-all text-sm font-medium text-text-secondary">
          ...
          <div className="relative">
            <button onClick={(e) => { e.stopPropagation(); setMoreOpen(!moreOpen); setTypeOpen(false); setSortOpen(false); }}
              className="flex h-9 shrink-0 items-center gap-2 rounded-lg bg-surface-card border border-border-subtle px-3.5 hover:border-primary/30 hover:shadow-sm transition-all text-sm font-medium text-text-secondary">
              <span className="material-symbols-outlined text-text-muted text-[16px]">more_horiz</span>More
              <span className="material-symbols-outlined text-text-muted text-[16px]">keyboard_arrow_down</span>
            </button>
            {moreOpen && (
              <div className="absolute top-full right-0 mt-2 bg-surface-card border border-border-subtle rounded-lg shadow-lg z-50 min-w-[200px] py-1">
                <button onClick={() => { setGroupByCompany(!groupByCompany); setMoreOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-text-secondary hover:bg-blue-50 transition-colors">
                  <span className="material-symbols-outlined text-[18px]">corporate_fare</span>{groupByCompany ? "Ungroup Contacts" : "Group by Company"}
                </button>
                <button onClick={() => { handleExportCSV(); setMoreOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-text-secondary hover:bg-blue-50 transition-colors">
                  <span className="material-symbols-outlined text-[18px]">download</span>Export to CSV
                </button>
                <button onClick={() => { handleImportCSV(); setMoreOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-text-secondary hover:bg-blue-50 transition-colors">
          ...
          {/* Add Contact */}
          <button onClick={openAddModal} className="flex items-center gap-2 px-4 py-2 text-white rounded-lg shadow-sm hover:opacity-90 transition-colors text-sm font-medium" style={{ backgroundColor: "#003366" }}>
      ...
      {loading ? (
              ...
              {viewMode === "grid" ? (
                ...
                <div className="bg-surface-card rounded-lg border border-border-subtle shadow-card overflow-hidden overflow-x-auto">
                  <table className="w-full min-w-[600px]"><thead><tr className="border-b border-border-subtle bg-slate-50/50">
                    ...
                    <th className="px-4 py-3 text-left text-[11px] font-bold text-text-muted uppercase tracking-wider">Email</th>
                    <th className="px-4 py-3 text-left text-[11px] font-bold text-text-muted uppercase tracking-wider">Last Contact</th>
                    <th className="px-4 py-3 text-left text-[11px] font-bold text-text-muted uppercase tracking-wider">Score</th>
                  </tr></thead><tbody>{grouped[company].map(renderContactRow)}</tbody></table>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="bg-surface-card rounded-lg border border-border-subtle shadow-card overflow-hidden overflow-x-auto">
          <table className="w-full min-w-[600px]"><thead><tr className="border-b border-border-subtle bg-slate-50/50">
            ...
            <th className="px-4 py-3 text-left text-[11px] font-bold text-text-muted uppercase tracking-wider">Last Contact</th>
            <th className="px-4 py-3 text-left text-[11px] font-bold text-text-muted uppercase tracking-wider">Score</th>
          </tr></thead><tbody>{contacts.map(renderContactRow)}</tbody></table>
        </div>
      )}

      {/* Pagination Info */}
      {!loading && contacts.length > 0 && (
        <div className="flex items-center justify-center text-sm text-text-muted py-2">
          Showing {contacts.length} of {totalContacts} contact{totalContacts !== 1 ? "s" : ""}
        </div>
      )}

      {/* Hidden file input for CSV import */}
      <input ref={importInputRef} type="file" accept=".csv" className="hidden" onChange={onImportFileSelected} />
      ...
      {deleteConfirmId && <DeleteConfirmModal onConfirm={() => handleDeleteContact(deleteConfirmId)} onCancel={() => setDeleteConfirmId(null)} />}
      {detailContactId && <DetailPanel contactId={detailContactId} onClose={() => setDetailContactId(null)} />}
    </div>
  );
}
