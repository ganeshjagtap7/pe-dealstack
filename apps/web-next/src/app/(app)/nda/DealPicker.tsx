...
// What the create flow needs from a deal: the id (to scope the NDA) and a
// human label for the modal subheader. We accept any fields the API returns
// and derive the label client-side so the picker stays resilient to schema
// drift. Same shape used by /graphs — this picker is deal-agnostic.
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              NDAs are scoped to a single deal — choose which one to draft for.
            </p>
          </div>
          <button
            onClick={onCancel}
              ...
              {filtered.map((d) => {
                ...
                return (
                  <li key={d.id}>
                    <button
                      onClick={() =>
                        onSelect({ id: d.id, label: buildBuilderLabel(d) })
                      }
                      className={cn(
  ...
  return (
    <div className="py-10 flex flex-col items-center justify-center text-center">
      ...
      <div className="text-xs text-slate-500 max-w-xs mt-1 mb-4">
        NDAs need a deal to live under. Create one to get started.
      </div>
      <Link
        href="/deals"
